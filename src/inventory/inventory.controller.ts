...
export class InventoryController {
  ...
  async findAll(@Param('userId') userId: string): Promise<any> {
    const [ventes, achats, articles] = await Promise.all([
      this.venteService.findByUserId(userId),
      this.achatService.findByUserId(userId),
      this.articleService.findByUserId(userId),
    ]);
    const inventory = [];
    for (const article of articles) {
      const qv = ventes.reduce(
        (acc, cur) => {
  ...
  async findOne(@Param('userId') userId: string, @Param('id') id: string) {
    const [ventes, achats, article] = await Promise.all([
      this.venteService.findByUserId(userId),
      this.achatService.findByUserId(userId),
      this.articleService.findOne(id),
    ]);
    const v = ventes.filter((v) =>
      v.produits.some((a) => a.ref === article.ref),
    );
    const a = achats.filter((v) =>
      v.produits.some((a) => a.ref === article.ref),
    );
